...
Поле определяет дллину заголовка и, соответственно, начало файловой таблицы. 
...
Кроме того после окончания полей заголока могут размещены дополнительные данные,
в этом случае начало индексной таблицы сдвигается (поле `index_ref`).

Так как размер указан без учёта константы MAGIC то начало файловой таблицы будет
определятся как `headlen + 4`.
